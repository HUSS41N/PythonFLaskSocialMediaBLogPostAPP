...
def create():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Post created!')
        return redirect(url_for('core.index')) #redirecting them to index page which is under core thats y core.index
    return render_template('create_post.html',form=form)

#viewing a blog post(view)
...
def update(blog_post_id):
    blog_post = BlogPost.query.get_or_404(blog_post_id)
    #checkimg if the author is correct 
    ...
    #To have that blog already filled up with the former data
